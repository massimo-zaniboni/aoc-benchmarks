...
function ValidMark($valid=FALSE){
   ...
   if ($valid){
      ...
      if (isset($_GET['m']) && strlen($_GET['m']) && strlen($_GET['m']) <= $bounds){
         ...
         if ($X && preg_match("/^[ a-zA-Z0-9_]+$/",$X)){ $mark = $X; $valid = TRUE; }
      }
   }
...
function ValidLangs($Langs,$valid=FALSE){
   return ValidWhiteList('w',$Langs,"^[a-zA-Z0-9_]+$",24,LANG_FULL,$valid);
}

function ValidTests($Tests,$valid=FALSE){
   return ValidWhiteList('ww',$Tests,"^[a-zA-Z0-9_]+$",32,TEST_NAME,$valid);
}
